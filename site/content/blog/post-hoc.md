...
Y = μ + ε<br>(日本語訳:)こいつは平均くらいなはずですが、平均からズレてる分については誤差としましょう。
...
Y = μ + ε<br>  ↓<br>Y = μ + A + ε <br>(日本語訳:)こいつは平均からAの影響分ズレています。それで説明できない部分は誤差としましょう。 <br>(さらに日本語訳:)平均値を見比べれば違いがあるかわかる。
...
Aは3つの条件だ！って思いこみがちですが、見方を考えれば正規分布から抽出したN=3のサンプルだと見ることもできるわけです。正規分布皇帝に万歳！後で出てきますがこれ超重要ポイントです。
...
SS/df = MS
<br>MS(A)=SS(A)/df(A)=312/2=156
<br>MS(ε)=SS(ε)/df(ε)=224/6=37.33
...
one-way between ANOVA がわかれば他は全てこの応用で、本質的には全く同じ操作をします。なので細かいことは教科書に投げて今回は大まかなことだけをかる～く触れます。

①モデルを考える
<br>②各項のSS→MSを求める
<br>③効果のMSをMS(ε)で割って効果ごとにF値を求める
<br>④求めたF値でF検定をする。
...
・このIVは明らかにDVに強い影響がある
<br>と
<br>・この群とこの群には明らかな差がある
<br>はpost-hocを使うと全く違う分析になってしまうということです。
