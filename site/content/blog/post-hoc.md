...
※わかりやすさ重視なので厳密性はゆるゆるです。決して教科書の代替ではないのでご了承ください。というか、教科書に突っ込む前にふわっと知っておきたかったまとめです。
...
はい、かなり説明端折ってます(※)。t検定はANOVAの特殊形なんですよ～みたいな話も端折ります。

t = {(Y-μ)√n}/σ t^2={(Y-μ)^2}n/MS(ε)
...
F値は分散の比のpdf(確率密度関数)です。だからMS(A)/MS(ε)と比を取ってF分布に照らし合わせれば確率分布がわかります。t検定の時と全く同じ考え方ですが、対立仮設はMS(A)>>MS(ε)なので上側だけ見る片側検定です。
...
※当たり前ですがこの分析は非現実的です。普通に考えてチュパカブラ3匹ずつじゃあんまり意味無いです。現実的には3人にはもっとたくさんのチュパカブラを育ててもらわないと統計的に意味のある結果は出ないでしょう。(具体的に何匹が妥当?という話は長くなるので省略)
