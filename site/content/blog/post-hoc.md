...
例えば任意の2つの実数を足し合わせて2で割ると、2つの値のバラツキは相殺されますよね。元の値わからなくなったら平均値を見ても元の2つの値はわかりません。元のバラツキの情報は消えます。当たり前の話ですね。

平均=4 → (2, 6), (3, 5), (1, 9)・・・無限に存在

例えば平均=4を考えたとき、もし元の値が(3, 5)ならだいたい4って言っても良さそうです。でも、もし元の値が(-2006, 2014)だとだいたい4って言うのは間違ってはいませんが、あんまり実用的じゃないですよね。

値のバラツキが大きいほど、平均だけで語る現実性はなくなっていきます。そこで世界をグループに分けてみたのです。

あるグループの平均を取ると、グループとは無関係なバラツキは全部相殺されて、そのグループが単体で全体の平均からどれくらいズレているかがわかります。当たり前の話ですね。

各グループ平均と総平均のズレを集めると、全体のバラツキの総量からグループとは無関係の”個人差”のバラツキを排除した純粋な”グループ”のバラツキになります。

この”グループのバラツキ”が大きければグループによって違いがありすぎるということなので、世界を総平均だけでは語れなくなってしまうのです。

だからこの”バラツキ”をどうコントロールするか、って話なんですが、、、バラツキを指標化した値は皆さんご存じ分散ですね。分散でモノゴトを評価する分析法なのでANOVA(分散分析、analysis of variance)と言います。
...
実際のANOVAに入る前にY=μ+A+εのモデルの意味を図示して確認してみましょう。Y=μ+A+εは行列式なので9匹のチュパカブラごとに1本ずつ固有の関係式が成り立ちます。Itamaeの113㎝のチュパカブラ(上表左端)に注目して見てみましょう。

Aは3つの条件だ！って思いこみがちですが、見方を考えれば正規分布に従うN=3のサンプルだと見ることもできるわけです(超重要ポイント)。正規分布皇帝に万歳！
...
分散の定義はY-μ(偏差)の二乗の合計をN数で割ることです。しかしデータ一つあたりに関するバラツキはt検定で使った不偏分散と同じでただＮで割るだけではつかえません。
...
でもSS(A)を標準化するには何で割ればいいか、、、と考えているとAの水準の数で割れば良さそうです。
...
> ※統計を学ぶ時、教科書上ではだいたい正規分布→χ²分布→F分布→T分布の順番で載っていますが、分析方法を学ぶ順番がだいたいT検定→ANOVA、となってるので皆チンプンカンプンになります。正規分布帝国を歩む最初の感動はやはり、ANOVAを通じて教科書通りにこれらの分布を一本の繋がった流れで理解することです。興味ある方は是非勉強してみてください。

F分布はF値(=分散の比)のpdf(確率密度関数)です。だからMS(A)/MS(ε)=Fを出してF分布に照らし合わせれば分布確率がわかります。これはt検定の時と全く同じ考え方ですが、ANOVAの場合の対立仮設はMS(A)>>MS(ε)なので上側だけ見る片側検定です。
...
これでモデルに依存しない効果量になりました。ちなみにこれもチュパカブラの例ではモデルの右辺にはそもそも効果が一つしかないので、η²=0.58=偏η²になります。どのみち大きいには変わらないので主効果は無視できないですね。
...
これであとは
SSを計算 → SS/df=MS → MS(各効果)/MS(ε)=F → 効果ごとにF検定実施&偏η²計算
