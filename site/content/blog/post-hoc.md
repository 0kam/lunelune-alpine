...
そもそも論、ANOVAというのはSSを我々の都合が良いように切り分けているだけです。ならば、主効果のSSをさらに分解すればGLMのまま条件間比較ができるのではないか?という考え方をすべきです。
...
そうなんです、ここではβXはバラメータマトリックス×デザインマトリックスです(それを言うなら正確にはμ = β₀X)。

βX = β₁X + β₂X + ・・・ + β(df)X

主効果を自由度の数の項に分解すれば条件間の比較ができるってことです!

ただし、うまく条件間比較をするためにはデザインマトリックスをうまい具合に設定する必要があります。この行列Xがコントラストで、どう設定するかが実験計画における非常に重要なポイントです。