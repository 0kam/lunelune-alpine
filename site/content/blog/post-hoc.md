...
データ一つあたりのバラツキは後で考えるとして、まずは各項のバラツキの絶対量を見てみましょう。それぞれの項について、各値を二乗して合計した二乗和で考えます。この二乗和のことを別名SS(平方和、sum of squares/squared sum)とも言います。
...
主効果Aは条件ごとにまとめて(これをプールすると言います)、それぞれで平均を出して、総平均μとの差分を取ります。これは言い換えればチュパカブラの個体差を無視した時、飼い主によってμからでれくらい変動するか、という値です。
...
よって、SS(total)を構成するSS(A)とSS(ε)の比を比較することで主効果Aの強さを評価しようという考え方をします。

でも、SSどうしを直接比較はできません。普通の分散ならSS(=全体のバラツキ量)をNで割れば一匹あたりのバラツキがわかって他のサンプルの分散と比較できます。これが標準化です。
...
なのでSSをdfで割ると不偏分散が出ます。これを皆MS(mean of squares, 平均平方)と呼んでます。
...
モデルのうち、A + B + AB の部分だけを引っ張り出すとこうなります。ABのdfはAとBのdfをかけると出てきます。
...
交互作用ASは存在します。実は交互作用は残差εに一致するんです。だから正確にはε＝０でASと書くこともできます。one-way betweenを考えると残差εはチュパカブラの個体差だけが影響しているバラツキなので誤差と言っても本質的にはε=Sということになります。これは各項の自由度を考えれば納得できます。自由度を説明していると終わらなくなるのですいませんが、この先は教科書に投げます。
