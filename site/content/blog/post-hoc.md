...
もし条件による差が全くなければ、全て同じ値、すなわち平均μになるはずです(Y=μ → 分散=0)。また、データ内にバラツキがあってもそれが全て条件による影響Ａだとしたら、各条件内では全く同じ値を取っているはずです(Y=μ+A → 分散=SS(A))。でも現実世界では、大抵はデータの中には効果Ａによっても説明できない誤差があってこれを残差εという呼びます(Y=μ+A+ε → 分散=SS(A)+SS(ε))。

もし、効果Aで強く説明できるとしたら、残差εのバラツキ、すなわちSSは小さくなり、データが効果Aだけで決まるとしたらSS(ε)は0になるはずです。逆に効果Aがデータの値とはあまり関係無かったらSS(ε)は大きくなり、効果Aの影響が全く無ければSS(ε)は全体の分散と一致するはずです。

さて、SS(ε)の大きさで効果Aを評価するといってもどうすればいいでしょうか?

ここで思い出してください。ここは由緒正しき正規分布帝国です。残差は正規分布に従うのです(拍手)。

(期待してる突っ込み)仮に同じSS(ε)だとしても、効果Aが例えば2条件の時と100条件の時じゃ同じって言えなくない??
