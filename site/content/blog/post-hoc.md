...
バラツキを指標化した値が皆さんご存じ分散(√を取れば標準偏差)です。分散を基準に評価する分析法なのでANOVA(分散分析、analysis of variance)と言います。

なんかこの調子で続けても電波濃度が上がるだけな気がするのでそろそろ数字を登場させましょう。例えば、板前氏とぽ氏とこのデータでone-way between ANOVAをしてみましょう。
...
これで全ての値を足し合わせると分散になります。分散を各項に分解したものをSS(平方和、sum of squares/squared sum)と言います。
