...
例えば2つの値を足し合わせて2で割ると、2つの値の差は相殺されて消えますよね。当たり前の話ですね。

あるグループの平均を取ると、グループ内のバラツキは全部相殺されて、グループ単体が全体の平均からどれくらいズレているかがわかります。

グループごとの平均を比べる、というのは全体のバラツキ全てから”個人差”のバラツキを排除してグループの影響力だけを比較することなのです。

バラツキを指標化した値が皆さんご存じ分散です。分散を基準に評価する分析法なのでANOVA(分散分析、analysis of variance)と言います。

***

## one-way between ANOVA

なんかこの調子で続けても電波濃度が上がるだけな気がするのでそろそろリアルな数字を登場させましょう。例えば、Itamae・ぽ・Johsonの3人にそれぞれチュパカブラの卵を3つずつ渡して育ててもらって、1年後に体長を測って成長を見たとしましょう。
...
分散の定義はY-μの二乗の合計をN数で割ることです。しかしデータ一つあたりに関するバラツキはt検定で使った不偏分散と同じでただＮで割るだけではつかえません。

データ一つあたりのバラツキは後で考えるとして、まずは各項のバラツキの絶対量を見てみましょう。

それぞれの項について、各値を二乗して合計した二乗和で考えます。この二乗和のことを別名SS(偏差平方和、sum of squared deviations/squared sum)とも言います。
...
主効果Aは条件ごとにまとめて(これをプールすると言います)、それぞれで平均を出します(水準平均)。
...
総平均μとの差分を取ります。これは言い換えればチュパカブラの個体差を無視した時、飼い主によってμからどれくらい変動するか、という値です。

A₁ = 118 - 120 = -2
A₂ = 114 - 120 = -6
A₃ = 128 -120 = 8

残差εはAを入れたあとに、最後に等式を丁度良く成立するように都合が良い数が入ります。だから”残された差”で残差と呼ばれていて、データの各点ひとつひとつにつき、ε=Y-μ-Aで定義されます。
...
Y = μ+ε → SS(A)=0, SS(total)=SS(ε)

②もしデータ内のバラツキが全て条件による影響Ａだけで説明される(=チュパカブラがどれくらい大きくなるかは誰が育てたかだけで決まる)としたら、各条件内では全く同じ値を取っているはずです。チュパカブラを育てた人が同じなら必ず同じ大きさに成長します。
Y = μ+A → SS(total)=SS(A), SS(ε)=0

よって、SS(total)を構成するSS(A)とSS(ε)の比で主効果Aの強さを評価しようという考え方をします。

でも、SS(A)とSS(ε)を直接比較するはできません。普通の分散ならSS(=全体のバラツキ量)をNで割れば一匹あたりのバラツキがわかって他のサンプルの分散と比較できます。これが標準化です。

でもSS(A)を標準化するには何で割ればいいか、、、と考えているとAの条件の数で割れば良さそうです。

ここでt検定でも出てきた不偏分散を思い出してください。推定値の分散はNではなく(N-1)で割ってましたよね?そうです、df(自由度)で割るのです。dfはそれぞれの項におけるN数的な存在なのです(これまた誤解を生みそうな表現ですが…)。

なのでSSをdfで割ると不偏分散が出ます。これをMS(mean of squares, 平均平方)と呼んでます。
...
> ※統計を学ぶ時、教科書上ではだいたい正規分布→χ²分布→F分布→T分布の順番で載っていますが、分析方法を学ぶ順番がだいたいT検定→ANOVA、となってるので皆チンプンカンプンになります。ANOVAを学ぶうえでの最初の感動はやはり教科書通りにこれらの分布を一連の流れで理解することなので興味ある方は是非勉強してみてください。
...
統計的仮説検定は考え方からして根本的に不完全な分析法です。特にN数に対して敏感です。なのでANOVAを行う際は効果量も一緒に見ましょう。

まず出てくる効果量は分散説明率、η²です。
...
もし仮にIVが増えてしまったらどうなるでしょうか?主効果Aが全く同じでも他のIVに関連するSSが加算されてSS(total)は大きくなってしまうのがわかると思います。だからモデルによってη²の解釈がモデルによって相対的に変わってしまうのです。

そこで偏η²が出てきます。

η² = SS(A) / {SS(A)+SS(ε)}

これでモデルに依存しない効果量になりました。ちなみにこれもチュパカブラの例ではモデルの右辺にはそもそも効果が一つしかないので、η²=偏η²です。

***

ここで、もし仮に、チュパカブラの例でANOVA有意となった場合を考えてみましょう。この時、飼い主によって違いが出るとまでは言えますが、飼い主の中で具体的に誰のチュパカブラが他の誰かより大きく成長するのかはわかりません。

そこで多くの場合、誰と誰の間に差があるのかを突き止めるために多重比較というものを行います。でも、ただt検定で比較を繰り返していくと、比較を重ねるごとにαエラーがかけられてしまいます。

例えば、有意水準をα=5%として、チュパカブラの例で飼い主の総当たり比較を行うと3回t-testを行うことになります。
...
> ※当たり前ですがこの分析は非現実的です。普通に考えてチュパカブラ3匹ずつじゃあんまり意味無いです。現実的には3人にはもっとたくさんのチュパカブラを育ててもらわないと統計的に意味のある結果は出ないでしょう。(具体的に何匹が妥当?という話は長くなるので省略します)

***

## two-way/within ANOVA

one-way between ANOVA がわかれば他は全てこの応用で、本質的には全く同じ操作をします。
...
two-way between ANOVA(対応の無い二元配置分散分析)変数Aと変数Bそれぞれの効果、それに加えてAとBの交互作用というものを考える必要があります。
...
SSを求める時だけ最初は手間取るかもしれませんが、それぞれの効果で水準ごとにプールします。

A → A₁ A₂ A₃ ・・
B → B₁ B₂ B₃ ・・
A×B → A₁⋂B₁ A₁⋂B₂ ・・A₂⋂B₁ A₂⋂B₂・・

先ほどの例をちょっと変えて、3人にチュパカブラを3匹ずつ育ててもらうかわりに1人につき、メキシコ産のチュパカブラ2匹とチリ産のチュパカブラ2匹の計4匹を育ててもらうことにしました。飼い主が効果A、産地が効果Bです。
...
これであとは
SSを計算→SS/df=MS→MS(各効果)/MS(ε)→効果ごとにF値→それぞれでF検定
です。
...
交互作用ASは存在します。実は交互作用は残差εに一致するんです。だから正確にはε＝０でASと書くこともできます。one-way betweenを考えると残差εはチュパカブラの個体差だけが影響しているバラツキなので誤差と言っても本質的にはε=Sということになります。

これは各項の自由度を考えれば納得できます。自由度を説明していると終わらなくなるのですいませんが、この先は教科書に投げます。
...
チュパカブラの例を考えると、もし事後検定で有意水準調整して3回t検定を行うと(Bonferroni法)、3つの比較のSSを合計すると元のモデルのSS(A)の1.5倍に膨れ上がって一致しません。
...
コーディングには、いくつか種類があります(例えば、名義尺度変数で使うダミーコーディング)。回帰式を崩さないコーディングがコントラストコーディングです。コントラストコーディングには以下の2つの特徴があるます。

①全ての条件が比較が比較に含まれていること
②各比較が独立であること

βXは自由度の数しか項がないのでコントラストコーディングでは総当たりの比較ができません。なのでコントラストをどう組むかはデータの背景にある論理的文脈を考慮する必要があります。

→逆に総当たりの比較をしなければならない場合はpost-hocによる多重比較が用いられます。
...
でも実際にこれでANOVAをしようと思うと困りますよね。SS(A)はSS(βX)ということになりますが、βがわかりません。でも心配しなくてもβの導出ができなくてもANOVAでは困りません(βの一般解はYにXの一般化逆行列をかけます)。

ここでSP(sum of products)というのを登場させます。
...
β₁X₁のSSとβ₂X₂のSSは大きさこそ違いますが、βの”重み”は同じですし、やはりpost-hocよりロジックが美しいですよね。
