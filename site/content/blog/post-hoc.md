...
※わかりやすさ重視なので厳密性はゆるゆるです。決して教科書の代替ではないのでご了承ください。というか、教科書を読む前にふわっと知っておきたかったことをまとめた感じです。
...
②もしデータ内のバラツキが全て条件による影響Ａだけで説明される(=チュパカブラがどれくらい大きくなるかは誰が育てたかだけで決まる)としたら、各条件内では全く同じ値を取っているはずです。チュパカブラを育てた人が同じなら必ず同じ大きさに成長します。(Y=μ+A → SS(total)=SS(A), SS(ε)=0)

SS(total)を構成するSS(A)とSS(ε)の比を比較することで主効果Aの強さを評価しようという考え方です。

でも、SSどうしを直接比較はできません。普通の分散ならSS(=全体のバラツキ量)をNで割れば一匹あたりのバラツキがわかります。これが標準化です。でも、SS(A)を標準化するには何で割ればいいか、、、と考えているとAの条件の数で割れば良さそうです。
...
ここで思い出してください。ここは由緒正しき正規分布帝国です。全ての項が正規分布なのです(拍手)。

ここからはだいぶ端折りながら駆け抜けますが、この辺の話はわかりやすい解説が山ほどあるので他にお任せします。

※統計を学ぶ時、教科書上ではだいたい正規分布→χ²分布→F分布→T分布の順番で載っていますが、分析方法を学ぶ順番がだいたいT検定→ANOVA、となってるので皆チンプンカンプンになります。ANOVAを学ぶうえでの最初の感動はやはり教科書通りに一連の流れでこれらの分布を理解することなのです。
