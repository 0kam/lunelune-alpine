...
データのバラツキがどこで生じたのかを考えたのです。系統的なバラツキがあれば、何かしらのシステマティックな影響を分離できるはずです。すなわち効果があるということです。
...
データ一つあたりのバラツキは後で考えるとして、まずは各項のバラツキの絶対量を見てみましょう。それぞれの項について、各値を二乗して合計した二乗和で考えます。この二乗和のことを別名SS(偏差平方和、sum of squared deviations/squared sum)とも言います。
...
これであとはSSを計算→SS/df→MS→効果のMSを一つずつをMS(ε)で割る→効果ごとにF検定です。
