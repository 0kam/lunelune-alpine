...
なんかこの調子で続けても電波濃度が上がるだけな気がするのでそろそろ数字を登場させましょう。例えば、Itamae・ぽ・Johsonの3人にそれぞれチュパカブラの卵を3つずつ渡して1年間育ててもらって1年後に体長を測って成長を見たとしましょう。このチュパカブラの体長のデータでone-way between ANOVA(対応の無い一元配置分散分析)をしてみましょう。
...
Σ{(Y-μ)^2}= Σ(A)^2 + Σ(ε)^2 →SS(total) = SS(A) + SS(ε)

※SS(total)/N = VAR(Y) = SD(Y)^2

そこで、こう考えます。

もし条件による差が全く影響していなければ、データは全て同じ値、すなわち平均μが並ぶだけになるはずです(Y=μ → SS(A),SS(ε)=0)。また、データ内にバラツキがあってもそれが全て条件による影響Ａだとしたら、各条件内では全く同じ値を取っているはずです(Y=μ+A → SS(total)=SS(A))。でも現実世界では、大抵はデータの中には効果Ａによっても説明できない誤差があってこれを残差εという呼びます(Y=μ+A+ε → SS(total)=SS(A)+SS(ε))。

もし、データを効果Aで強く説明できるとしたら、残差SS(ε)は小さくなり、データが効果Aだけで決まるとしたらSS(ε)=0になるはずです。

逆に効果Aがデータの値とはあまり関係無かったらSS(ε)は大きくなり、効果Aの影響が全く無ければSS(A)=0となり、SS(ε)=SS(total)となるはずです。
...
(期待してる突っ込み)主効果と被験者効果の交互作用ASは無くても良いの?

交互作用ASは存在します。実は交互作用は残差εに一致するんです。これは各項の自由度を考えれば納得できます。
