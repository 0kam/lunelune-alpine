...
でも実際にこれでANOVAをしようと思うと困りますよね。SS(A)はSS(βX)ということになりますが、βはわかりません。でも心配しなくてもβの導出ができなくてもANOVAでは困りません(βの導出はYにXの一般化逆行列をかけます)。

ここまでは数字や文字が違うだけで先ほど見た分析と全く同じ形です。ここでさらに、3条件(one-way between ANOVA)についてみてみましょう。
