...
ANOVAはGLM(一般線型モデル)の特殊形です。post-hoc法が論理的に誤り、というのはANOVAの回帰モデルを全く無視した解析を無理に追加で組み合わせているからなのです。

そもそも論、ANOVAというのはSSを我々の都合が良いように切り分けているだけです。ならば、主効果のSSをさらに分解すればGLMのまま条件間比較ができるのではないか?という考え方が実験計画における第一選択であるべきです。
...
コーディングには名義尺度のデータを食わせるときに使うダミーコーディングなど、いくつか種類がありますが、ここで使われるのがコントラストコーディングです。コントラストコーディングには2つの特徴があるます。全ての条件を比較をしていること、そして各条件が独立であることです。

説明だけじゃピンとこないと思うので、まずは、2条件(independent t-test)について考えてみましょう。
...
ここで、Xは列ベクトルですので実際にはこういうことになります。
...
さらに、3条件(one-way between ANOVA)についてみてみましょう。
