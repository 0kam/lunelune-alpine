...
例えば任意の2つの実数を足し合わせて2で割ると、2つの値のバラツキは相殺されますよね。もし元の値わからなくなったら平均値だけ見ても元の2つの値はわかりません。平均を取ると元のバラツキの情報は消えます。当たり前の話ですね。
...
この”グループのバラツキ”が大きければグループによって違いがありすぎるということなので、世界を総平均だけでは語れないと言うことができます。これが分散分析の目的です。
...
例として、Johsonの124㎝のチュパカブラに注目して見てみましょう。
...
と
・この群とこの群には明らかな差がある
はpost-hocを使うと全く違う分析になってしまうということです。
...
> ※一般的には回帰分析として捉えるとSS(total)→SS(Y)、SS(A)→SS(reg.)などと表記を変えることがありますが、式の中身は全く同じです。ここでは混乱を避けるために上の表記で考えます

このあたりの式変形は教科書で省略されているので丁寧に追ってみることにします。(興味ない人は飛ばして下さい)
...
= Σ{X²Y² - 2X²Y(μ+ε) + X²(μ+ε)²}

= Σ(XY)² - 2Σ{X²Y(μ+ε)} + Σ{X(μ+ε)}²

= (ΣXY)² = (SP(XY))²

ここで、ΣXYをSP(XY)と表記します。二変数の積の合計をSP(sum of products)と呼びます。

SS(βX) = (SP(XY))² / SS(X)

これで今までと同じようにdfで割ってF値を出すことができます。
