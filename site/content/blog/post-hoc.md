...
Σ{(Y-μ)^2} = Σ{(A)^2} + Σ{(ε)^2} 同じ式を書き換えると SS(total) = SS(A) + SS(ε)

![](/img/1waytable2.jpg)

主効果Aは条件ごとにまとめて(これをプールすると言います)、それぞれで平均を出します(水準平均)。

総平均μ = 120平均(Itamae) = 118平均(ぽ) = 114平均(Johson) = 128

総平均μとの差分を取ります。これは言い換えればチュパカブラの個体差を無視した時、飼い主によってμからどれくらい変動するか、という値です。

A₁ = 118 - 120 = -2A₂ = 114 - 120 = -6A₃ = 128 -120 = 8

残差εはAを入れたあとに、最後に等式を丁度良く成立するように都合が良い数が入ります。だから”残された差”で残差と呼ばれていて、ε=Y-μ-Aで定義されます。
...
コーディングには、いくつか種類があります(例えば、名義尺度変数で使うダミーコーディング)。ANOVAで使うのはコントラストコーディングです。コントラストコーディングには2つの特徴があるます。
