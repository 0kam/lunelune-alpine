...
βの導出ができなくてもANOVAでは困りません(導出はYにXの一般化逆行列をかけます)。

ここまでは従来の解析と同じです。ここでさらに、3条件(one-way between ANOVA)についてみてみましょう。
...
まだまだ2-wayだのwithinだのありますが、全部同じ考え方です。そしてさらには混合モデルだの階層モデルだのANCOVAだのGLMの底はまだまだ深いです泣。
