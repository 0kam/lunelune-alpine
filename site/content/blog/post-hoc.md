...
もし条件による差が全くなければ、全て同じ値、すなわち平均μになるはずです。また、データ内にバラツキがあってもそれが全て条件による影響Ａだとしたら、各条件内では全く同じ値を取っているはずです。でも現実にはそうならず、大抵データの中には条件で説明できない影響が含まれていてバラツキが出ます。独立変数の効果Ａによっても説明できないバラツキが誤差εで残差という呼びます(この場合は主に個人差ってやつです)。

バラツキは各値の平均値からのズレに注目します。平均からどれくらいずれているかが大事であり、平均に対して大きいか小さいかは気にしないので平均値との差を2乗して足し合わせます。これをSS(平方和、sum of squares/squared sum)と言いますが、要は各項における分散のことです。分散を基準に評価する分析法なのでANOVA(分散分析、analysis of variance)です。
