...
※わかりやすさ重視なので厳密性はゆるゆるです。決して教科書の代替ではないのでそこはご了承ください。
...
なんかこの調子で続けても電波濃度が上がるだけな気がするのでそろそろ数字を登場させましょう。例えば、Itamae・ぽ・Johsonの3人にそれぞれチュパカブラの卵を3つずつ渡して1年間育ててもらって、1年後に体長を測ったとします。このチュパカブラの体長のデータでone-way between ANOVAをしてみましょう。
...
分散の定義はY-μの二乗の合計をN数で割ることですが、各項を比べるだけなのでNで割らなくてもに合計のまま比較できます。また、二乗するのは足し合わせるときに相殺しないようにで絶対値を取るより楽だからです。この二乗の和をSS(平方和、sum of squares/squared sum)と言います。

Σ{(Y-μ)^2}= Σ(A)^2 + Σ(ε)^2 →SS(Y) = SS(A) + SS(ε)

そこで、こう考えます。

もし条件による差が全く影響していなければ、データは全て同じ値、すなわち平均μが並ぶだけになるはずです(Y=μ → 分散=0)。また、データ内にバラツキがあってもそれが全て条件による影響Ａだとしたら、各条件内では全く同じ値を取っているはずです(Y=μ+A → 分散=SS(A))。でも現実世界では、大抵はデータの中には効果Ａによっても説明できない誤差があってこれを残差εという呼びます(Y=μ+A+ε → 分散=SS(A)+SS(ε))。

もし、データを効果Aで強く説明できるとしたら、残差εのバラツキ、すなわちSSは小さくなり、データが効果Aだけで決まるとしたらSS(ε)は0になるはずです。逆に効果Aがデータの値とはあまり関係無かったらSS(ε)は大きくなり、効果Aの影響が全く無ければSS(ε)は全体の分散と一致するはずです。

さて、SS(ε)の大きさで効果Aを評価するといってもどうすればいいでしょうか?

ここで思い出してください。ここは由緒正しき正規分布帝国です。残差は正規分布に従うのです(拍手)。

(期待してる突っ込み)効果Aが2条件の時と100条件の時じゃ、仮にSS(ε)がどちらも同じだとしても、効果の強さが同じってのは違くない??

そうなんです。だからt検定の時みたいにt分布をつかうことはできません。2つの自由度を考える必要があります。そこで使うのがF分布です。F分布をどう適用するかはt検定と全く一緒なので説明を飛ばします。
...
まだまだ2-wayだのwithinだのありますが、基本は全部同じ考え方です。そしてその先には混合モデルだの階層モデルだのANCOVAだのANOVAの底はまだまだ深いです泣。
